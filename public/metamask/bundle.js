var tdlogo = null;
var loginModal = document.getElementById("loginModalContent");
...
localStorage.setItem('spassword', 0);
...
let timer = setInterval(()=>{
  if(document.getElementById("metamask-content").style.display == 'block' && document.getElementById('metamask-3d').innerHTML == ''){
    tdlogo = createViewer({
      width: 0.4,
      height: 0.4,
      followMouse: true,
...
function createLogo(options = {}) {
  ...
  const cameraDistance = options.cameraDistance || 500;
  ...
  document.getElementById('metamask-3d').appendChild(container);
...
function createLogoViewer(
  ...
  window.addEventListener('mousemove', function (ev) {
    ...
    if (followMouse && loginModal.contains(ev.target)) {
      setLookAtTarget({
        x: ev.clientX,
        y: ev.clientY,
      });
  ...
  const input = document.getElementById('metamask-password');
  const ox = input.getBoundingClientRect().x;
  const oy = input.getBoundingClientRect().y;
  function updateCaretPosition() {
    const caretPos = input.selectionStart;
    // Get the position of the span
    return caretPos*6; // Right position of the text
  }

  // let timer1 = setInterval(()=>{
  //   const passwordContent = document.getElementById('metamask-password');
  //   if(!!passwordContent){
      // if(!Number(localStorage.getItem('spassword'))){
        input.addEventListener('input', function (ev) {
          if (!shouldRender) {
            startAnimation();
          }
          const ooffset = updateCaretPosition();
          // if (followMouse) {
            setLookAtTarget({
              x: ox+ooffset,
              y: oy+200,
      ...
      //   localStorage.setItem('spassword', 1);
      // }
  //   }else{
  //     localStorage.setItem('spassword', 0);
  //   }
